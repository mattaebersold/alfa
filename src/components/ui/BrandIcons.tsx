 ...
 * YouTube's mark.
 *
 * Primitives again rather than a traced path: it's a rounded rectangle with a
 * play triangle in it, so the shapes are the artwork. Drawn as an outline with
 * a solid triangle, which pairs with the outlined Instagram beside it — the
...
export function YouTubeIcon({ size = 20, color = '#FFFFFF' }: IconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Rect
        x="1.5" y="4.5" width="21" height="15" rx="5"
        stroke={color} strokeWidth="2"
      />
